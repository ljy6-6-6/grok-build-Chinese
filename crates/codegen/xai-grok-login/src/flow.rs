...
pub(super) async fn run_auth_flow_steps(
    ...
    anyhow::bail!(
        "No OAuth2 configuration available. Run `grok-zh login` to authenticate, or contact your administrator if you use enterprise SSO."
    )
}
...
async fn run_cli_login_steps(
    ...
    .await
    {
        ...
        .await?;
        if did_auth {
            report_signed_in_with_locale(&auth, locale);
        }
        auth
    } else {
...
pub(crate) fn report_signed_in_with_locale(
    auth: &GrokAuth,
    locale: &xai_grok_locale::LocaleContext,
) {
    eprint!("\r\x1b[K");
    let message = match auth.email.as_deref() {
        Some(email) => locale
            .named_text("auth.cli.signed_in_as", "✓ Signed in as {email}")
            .replace("{email}", email),
        None => locale
            .named_text("auth.cli.signed_in", "✓ Signed in")
            .into_owned(),
    };
    eprintln!("{message}");
}
