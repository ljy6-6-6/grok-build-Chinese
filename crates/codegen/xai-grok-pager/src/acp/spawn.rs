    ...
    let outcome = match rx.recv_timeout(quiet) {
        ...
        Err(RecvTimeoutError::Timeout) => {
            if std::io::stderr().is_terminal() {
                notice_shown = write_join_notice_with_notice(&mut std::io::stderr(), join_notice);
            }
...
fn write_join_notice(w: &mut impl Write) -> bool {
    write_join_notice_with_notice(w, JOIN_NOTICE)
}

fn write_join_notice_with_notice(w: &mut impl Write, notice: &str) -> bool {
    crate::best_effort_stderr::write_line(w, notice)
}
