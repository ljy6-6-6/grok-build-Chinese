...
impl AppView {
    ...
    pub fn set_appearance(&mut self, config: AppearanceConfig) {
        ...
        for agent in self.agents.values_mut() {
            ...
            agent.set_plugins_visible_recursive(!config.disable_plugins);
...
// The current upstream suite lives in `app_view_tests.rs`; keep the historical
// inline copy out of compilation so the consent-aware fixtures are authoritative.
#[cfg(any())]
pub(crate) mod legacy_tests {
    use super::*;
    use crate::acp::model_state::ModelState;
