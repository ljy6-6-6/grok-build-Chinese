...
pub(super) fn render_header(
    ...
    let locale = state.ui_locale().clone();
        ...
        status.push(
            ...
            Line::from(vec![
                ...
                Span::styled(
                    format!(
                        " {count} {}",
                        locale.named_text(&format!("dashboard.header.{label}"), label)
                    ),
                    bg.fg(theme.gray),
                ),
            ]),
    ...
    let LocationParts {
        ...
        is_worktree,
        cwd_display,
    } = location_parts_with_locale(&state.cwd, Some(&locale));
...
pub(super) fn render_actions_row(
    ...
    let new_agent_label = if worktree_armed {
        locale.named_static_text(
            "dashboard.action.new_agent_worktree",
    ...
    let worktree_label = if worktree_armed {
        ...
        locale.named_static_text("dashboard.action.worktree", "Worktree")
    };
