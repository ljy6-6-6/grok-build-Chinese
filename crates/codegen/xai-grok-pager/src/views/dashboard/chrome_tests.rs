...
#[test]
fn zh_localization_dashboard_actions_keep_painted_hit_areas() {
    ...
    let mut buf = Buffer::empty(area);
    render_actions_only(&mut buf, area, &theme, &mut state, true);
    let rendered = buf_to_text(&buf);
    assert!(rendered.contains("新建智能体"), "{rendered}");
    let rect = state
        .new_agent_button_hit
        .rect
        .expect("painted button must have a click target");
    assert!(rect.x >= area.x && rect.right() <= area.right());
    assert_eq!(
        rect.width as usize,
        unicode_width::UnicodeWidthStr::width("+ 新建智能体")
    );
    let locale_en = crate::locale::LocaleContext::default();
    state.set_ui_locale(Some(&locale_en));
    render_actions_only(&mut buf, area, &theme, &mut state, true);
    assert!(buf_to_text(&buf).contains("+ New Agent"));
}
