...
impl Section {
    ...
    fn label_with_locale(self, locale: Option<&crate::locale::LocaleContext>) -> &'static str {
        let (id, english) = match self {
            Section::Subagents => ("dock.section.subagents", "Subagents"),
            Section::Tasks => ("dock.section.tasks", "Tasks"),
            Section::Watchers => ("dock.section.watchers", "Watchers"),
            Section::Queued => ("dock.section.queued", "Queued"),
        };
        locale
...
pub fn render(buf: &mut Buffer, area: Rect, theme: &Theme, data: &DockData) {
    render_with_locale(buf, area, theme, data, None);
}
        ...
        match item {
            DockItem::Header(section) => {
                ...
                let line = section_header(
                    ...
                    area.width,
                    expanded,
                    section.label_with_locale(locale),
                    count,
                );
            ...
            DockItem::RevealRemaining(section) => {
                ...
                let line = Line::from(Span::styled(
                    format!(
                        "{}{arrow} {}",
                        &MORE_INDENT[..indent_len],
                        locale
                            .map(|l| l
                                .named_text("dock.show_more", "show {count} more")
                                .into_owned())
                            .unwrap_or_else(|| "show {count} more".to_string())
...
fn paint_row_with_locale(
    ...
    let stop_label = locale
        .map(|l| l.named_static_text("dock.stop", "[stop]"))
        .unwrap_or("[stop]");
    // Both supported locales use exactly six display cells; keep the upstream hit rect.
    debug_assert_eq!(stop_label.width(), section.kill_label().width());
    ...
    if show_actions {
        ...
        if row.killable {
            meta_spans.push(Span::styled(stop_label, Style::default().fg(kill_color)));
        }
    }
