...
impl MetadataField {
    ...
    pub(super) fn filtered_variants(self, query: &str) -> Vec<usize> {
        self.filtered_variants_with_locale(query, &crate::locale::LocaleContext::default())
    }

    pub(super) fn filtered_variants_with_locale(
        self,
        query: &str,
        ...
        self.variant_labels()
            ...
            .filter(|(_, label)| {
                label.to_lowercase().contains(&query)
                    || super::localization::text(locale, label)
                        .to_lowercase()
                        .contains(&query)
            })
            .map(|(index, _)| index)
...
impl FeedbackModalState {
    ...
    pub(super) fn render_metadata_rows(
        ...
        for (index, field) in fields.iter().enumerate() {
            ...
            let Some(text) = self.metadata.field_text_with_locale(*field, &self.locale) else {
                continue;
            };
    ...
    pub(super) fn handle_enum_picker_event(
        ...
        let filtered = field.filtered_variants_with_locale(picker.state.query(), &self.locale);
    ...
    pub(super) fn render_enum_picker(
        ...
        let labels: Vec<_> = picker
            ...
            .variant_labels()
            .into_iter()
            .map(|label| super::localization::text(&self.locale, label))
            .collect();
        let filtered = picker
            .field
            .filtered_variants_with_locale(picker.state.query(), &self.locale);
