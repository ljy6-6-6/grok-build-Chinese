...
impl FeedbackModalMetadata {
    ...
    fn field_text(&self, field: MetadataField) -> Option<String> {
        self.field_text_with_locale(field, &crate::locale::LocaleContext::default())
    }

    fn field_text_with_locale(
        &self,
        field: MetadataField,
        ...
        match field {
            MetadataField::Type => Some(match &self.r#type {
                Some(value) => format!(
                    "{}: {}",
                    localization::text(locale, "Type"),
                    localization::text(locale, value.label())
                ),
                None if self.draft_id.is_some() => format!(
                    "{}: {}",
                    localization::text(locale, "Type"),
                    localization::text(locale, "(choose)")
                ),
                None => return None,
            }),
            ...
            MetadataField::Failure => self.failure_mode.as_ref().map(|value| {
                format!(
                    "{}: {}",
                    localization::text(locale, "Failure"),
                    localization::text(locale, value.label())
                )
            }),
        }
...
impl FeedbackModalState {
    pub fn new(open: OpenFeedbackModal) -> Self {
        Self::new_with_locale(open, &crate::locale::LocaleContext::default())
    }

    pub fn new_with_locale(
        mut open: OpenFeedbackModal,
        locale: &crate::locale::LocaleContext,
    ) -> Self {
        ...
        Self {
            ...
            error: (rejected_images > 0).then(|| {
                locale
                    .named_text(
                        "feedback.modal.images_rejected",
                        "Dropped {count} invalid image(s).",
                    )
                    .replace("{count}", &rejected_images.to_string())
            }),
    ...
    pub fn handle_key(&mut self, key: &KeyEvent) -> FeedbackModalOutcome {
        ...
        match self.composer.route_enter(key) {
            ...
            EnterOutcome::Submit => {
                self.error =
                    Some(localization::text(&self.locale, FEEDBACK_EMPTY_SUBMIT_ERROR).to_string());
                return FeedbackModalOutcome::Changed;
            }
