...
impl FeedbackModalState {
    pub fn render(
        ...
        let shortcuts: Vec<_> = shortcuts
            .iter()
            .map(|s| Shortcut {
                label: super::localization::text(&locale, s.label),
                clickable: s.clickable,
                id: s.id,
            })
            .collect();
        let tabs = [
            super::localization::text(&locale, "Write"),
            super::localization::text(&locale, "Drafts"),
        ];
        let mut config = Self::window_config(&shortcuts, compact, trace_selected.is_some());
        config.title = super::localization::text(&locale, "Feedback");
        ...
        let prompt_result = self.composer.draw(
            buf,
            content,
            Some(area),
            &Self::prompt_style_with_locale(&locale),
            None,
            None,
        );
    ...
    fn render_drafts(
        ...
        let mut shortcuts = vec![
            Shortcut {
                label: super::localization::text(&locale, "↑↓/j k move"),
                clickable: false,
                id: 0,
            },
            Shortcut {
                label: super::localization::text(&locale, "/ search"),
                clickable: false,
                id: 0,
            },
            Shortcut {
                label: super::localization::text(&locale, "Enter open"),
                clickable: false,
                id: 0,
            },
            Shortcut {
                label: super::localization::text(&locale, "d delete"),
                clickable: false,
                id: 0,
            },
        ];
        ...
        let mut config = Self::window_config(&shortcuts, compact, false);
        config.title = super::localization::text(&locale, "Feedback");
        config.tabs = Some(&tabs);
        let areas = modal_window::render_modal_window(buf, area, &mut self.window, &config, theme)?;
        ...
        if self.discard_confirm.is_some() {
            ...
            buf.set_stringn(
                ...
                super::localization::text(&locale, "y discard  |  n cancel"),
                content.width as usize,
                dim,
            );
            return Some(FeedbackModalRender {
        ...
        if let Some(draft_id) = self.delete_confirm.as_ref() {
            let prompt = if self.metadata.draft_id.as_ref() == Some(draft_id) {
                ...
                super::localization::text(&locale, "Delete this feedback draft?")
            };
            ...
            buf.set_stringn(
                ...
                if self.draft_delete.is_some() {
                    super::localization::text(&locale, "Deleting…")
                } else {
                    super::localization::text(&locale, "y delete  |  n cancel")
                },
                content.width as usize,
                dim,
            );
        ...
        match &self.drafts {
            DraftsState::Unloaded => {
                buf.set_stringn(
                    content.x,
                    content.y,
                    super::localization::text(&locale, "Open Drafts to load saved feedback."),
                    content.width as usize,
                    dim,
                );
            }
            DraftsState::Loading { .. } => {
                buf.set_stringn(
                    content.x,
                    content.y,
                    super::localization::text(&locale, "Loading drafts…"),
                    content.width as usize,
                    dim,
                );
            }
            DraftsState::Browse {
                    ...
                    .take(capacity)
                {
                    ...
                    let failure = draft
                        .failure_mode
                        .map(|value| {
                            format!(" · {}", super::localization::text(&locale, value.label()))
                        })
                        .unwrap_or_default();
                    let r#type = draft.r#type.map_or(
                        super::localization::text(&locale, "Unclassified"),
                        |value| super::localization::text(&locale, value.label()),
                    );
                    let task = draft
                        .task_category
                        .map_or(super::localization::text(&locale, "Other"), |value| {
                            super::localization::text(&locale, value.label())
                        });
                ...
                if rendered == 0 && y < content.bottom() {
                    let empty = if rows.is_empty() {
                        super::localization::text(&locale, "No drafts.")
                    } else {
                        super::localization::text(&locale, "No matching drafts.")
                    };
        ...
        for (index, choice) in FeedbackTraceChoice::ALL.into_iter().enumerate() {
            ...
            line(
                ...
                &format!(
                    "{marker} {}. {}",
                    index + 1,
                    super::localization::text(&locale, choice.label())
                ),
                style,
            );
        }
    ...
    pub(super) fn prompt_style() -> PromptStyle {
        Self::prompt_style_with_locale(&crate::locale::LocaleContext::default())
    }

    pub(super) fn prompt_style_with_locale(locale: &crate::locale::LocaleContext) -> PromptStyle {
        PromptStyle {
            show_prefix: false,
            placeholder_when_focused: true,
            placeholder_override: Some(super::localization::text(locale, "Tell us what happened")),
