...
#[test]
fn zh_localization_feedback_trace_is_one_shot_and_keeps_wire_taxonomy() {
    let locale = crate::locale::LocaleContext::new(crate::locale::ResolvedLocale {
        locale: crate::locale::UiLocale::ZhCn,
        source: crate::locale::LocaleSource::ProductDefault,
    });
    let report = "Keep this exact user report: /feedback {payload}";
    let mut modal = FeedbackModalState::new_with_locale(
        OpenFeedbackModal {
    ...
    assert_eq!(modal.composer.text(), report);
    let wire_before = modal.metadata.structured_feedback();
    modal.begin_trace_step();
    let rendered = render_to_text(&mut modal);
    for label in ["反馈", "本次会话", "仅发送反馈", "不再询问"] {
        assert!(rendered.contains(label), "{rendered}");
    }
    assert_eq!(modal.metadata.structured_feedback(), wire_before);
    assert_eq!(
        modal.handle_key(&key(KeyCode::Enter, KeyModifiers::NONE)),
        FeedbackModalOutcome::Submit
    );
...
#[test]
fn zh_localization_feedback_picker_matches_both_languages_without_reindexing() {
    let locale = crate::locale::LocaleContext::new(crate::locale::ResolvedLocale {
        locale: crate::locale::UiLocale::ZhCn,
        source: crate::locale::LocaleSource::ProductDefault,
    });
    let en = MetadataField::Type.filtered_variants("Bug");
    let zh = MetadataField::Type.filtered_variants_with_locale("错误", &locale);
    assert_eq!(zh, en);
    assert_eq!(
        MetadataField::Type.filtered_variants_with_locale("Bug", &locale),
        en
    );
}
