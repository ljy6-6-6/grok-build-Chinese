...
pub(crate) fn worktree_badge_with_locale(
    theme: &Theme,
    locale: Option<&crate::locale::LocaleContext>,
) -> Span<'static> {
    let badge = locale
        .map(|l| l.named_static_text("welcome.location.worktree_badge", "worktree "))
        .unwrap_or("worktree ");
    Span::styled(badge, Style::default().fg(theme.accent_user))
}
...
fn location_parts_from_with_locale(
    cwd: &Path,
    ...
    let branch = info.and_then(|i| i.branch).map(|b| {
        if b.is_empty() {
            locale
                .map(|l| l.named_static_text("welcome.location.detached", "detached"))
                .unwrap_or("detached")
                .to_owned()
        } else {
            b
...
pub(crate) fn location_parts_with_locale(
    cwd: &Path,
    locale: Option<&crate::locale::LocaleContext>,
) -> LocationParts {
    location_parts_from_with_locale(cwd, git_info::cwd_git_info_lazy(cwd), locale)
}
