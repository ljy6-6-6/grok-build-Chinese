...
pub const AUTH_ERROR_SESSION_EXPIRED: &str =
    "Session expired. Run `grok-zh login` to re-authenticate.";

pub const AUTH_ERROR_API_KEY: &str = "Authentication failed. Run `grok-zh login`, set XAI_API_KEY, or add api_key to ~/.grok/config.toml.";
...
pub const PREFERRED_OIDC_UNAVAILABLE: &str =
    "preferred_method=oidc but no session is available. Run `grok-zh login` to authenticate.";
