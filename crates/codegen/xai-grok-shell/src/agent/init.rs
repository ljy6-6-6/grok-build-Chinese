...
pub async fn apply_post_login_config(
    ...
    apply_post_login_config_with_locale(authenticated, &xai_grok_locale::LocaleContext::default())
        .await
}

pub async fn apply_post_login_config_with_locale(
    authenticated: xai_grok_login::GrokAuth,
    locale: &xai_grok_locale::LocaleContext,
) -> anyhow::Result<()> {
    let outcome = crate::managed_config::post_login_sync(Some(authenticated)).await;
    if let Some(notice) = post_login_config_notice(locale, outcome) {
        eprintln!("{notice}");
    }
    Ok(())
}
