...
#[test]
fn prompt_suggest_catalog_check_honors_allowlist() {
    let cfg = config_from_toml(
        r#"
            [models]
            allowed_models = ["keep-*"]
    ...
    let mgr = ModelsManagerBuilder::new(
        None,
        catalog,
        acp::ModelId::new("keep-key"),
        auth_manager,
        cfg,
    )
    .build();

    assert!(mgr.model_selectable_in_catalog("keep-key"));
    assert!(mgr.model_selectable_in_catalog("keep-slug"));
    assert!(!mgr.model_selectable_in_catalog("blocked-key"));
    assert!(!mgr.model_selectable_in_catalog("blocked-slug"));
    assert!(!mgr.model_selectable_in_catalog("missing"));
}
